...
const PoliticasAdmin = () => {
    ...
    const cargarPoliticas = async () => {
        try {
            ...
            const response = await fetch('https://backend-iota-seven-19.vercel.app/api/politicas/admin', {
                headers: {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const url = editingItem 
                ? `https://backend-iota-seven-19.vercel.app/api/politicas/admin/${editingItem._id}`
                : 'https://backend-iota-seven-19.vercel.app/api/politicas/admin';
    ...
    const handleDelete = async (id) => {
        ...
        try {
            ...
            const response = await fetch(`https://backend-iota-seven-19.vercel.app/api/politicas/admin/${id}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
        ...
        <Container fluid>
                                ...
                                <tbody>
                                    {politicas.map((politica) => (
                                        <tr key={politica._id}>
                                            <td>
                                                <div>
                                                    ...
                                                    <small style={{ color: '#666' }}>
                                                        {politica.descripcion.length > 100 
                                            ...
                                            <td>
                                                <Badge bg={getTipoColor(politica.tipo)}>
                                                    {getTipoLabel(politica.tipo)}
                                                </Badge>
                                            </td>
                                            <td>
                                                <Badge bg={politica.activo ? 'success' : 'secondary'}>
                                                    {politica.activo ? 'Activo' : 'Inactivo'}
                                                </Badge>
                                            </td>
                                            <td>{politica.orden}</td>
                                            <td>
                                                {new Date(politica.createdAt).toLocaleDateString('es-ES')}
                                            </td>
                                            <td>
            ...
            <Modal show={showModal} onHide={handleCloseModal} size="lg">
                <Modal.Header closeButton>
                    <Modal.Title>
                        {editingItem ? 'Editar Política' : 'Nueva Política'}
                    </Modal.Title>
                </Modal.Header>
                            ...
                            <Col md={6}>
                                <Form.Group className="mb-3">
                                    ...
                                    >
                                        ...
                                        <option value="uso">Términos de Uso</option>
                                    </Form.Select>
                                </Form.Group>
                            </Col>
                        </Row>
