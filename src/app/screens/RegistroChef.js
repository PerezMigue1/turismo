...
const RegistroChef = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!token || !idUsuario) {
            setMensaje('⚠️ Debes iniciar sesión para registrar un chef.');
        ...
        try {
            await axios.post('https://backend-iota-seven-19.vercel.app/api/contactoChef', data, {
                headers: {
            ...
            // Actualizar el usuario con el rol de chef
            const updatedUser = { ...usuarioLogeado, rol: 'chef' };
            localStorage.setItem('user', JSON.stringify(updatedUser));

            setMensaje('✅ Registro exitoso');
            setFormData({
                nombre: '', correo: '', telefono: '', ubicacion: '',
                especialidad: 'Gastronomia', descripcion: '', imagenPerfil: null,
                facebook: '', instagram: '', whatsapp: ''
            });
            setImagePreview(null);
        } catch (error) {
            console.error("❌ Backend response:", error.response?.data || error.message);
            setMensaje('❌ Error al registrar chef');
        } finally {
            setIsSubmitting(false);
        }
    };
            ...
            <Container>
                <div className="text-center mb-5">
                    ...
                    <p className="lead text-muted">
                        Únete a nuestra comunidad gastronómica y comparte tu talento culinario
                    </p>
                </div>
                        ...
                        <Form onSubmit={handleSubmit}>
                            ...
                            <div className="mb-5">
                                ...
                                <Row>
                                    <Col md={6}>
                                        <Form.Group className="mb-4">
                                            <Form.Label className="fw-bold">🍽️ Especialidad Culinaria</Form.Label>
                                            <Form.Control 
                                                name="especialidad" 
                                ...
                                <Form.Group className="mb-4">
                                    ...
                                    <Form.Control 
                                        ...
                                        onChange={handleChange}
                                        placeholder="Comparte tu experiencia como chef, qué platillos te apasionan y qué tipo de cocina dominas..."
                                        className="border-0 shadow-sm"
                                        style={{ borderRadius: '10px', padding: '12px' }}
                                    />
                                </Form.Group>
